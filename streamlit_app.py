...
raw_model_url = 'https://raw.githubusercontent.com/farhan0404/Deep_Learning/main/my_model_2.keras'

# Fetch the raw content of the model architecture file
response = requests.get(raw_model_url)
model_bytes = BytesIO(response.content)

# Load the model architecture from JSON or YAML
model_architecture = model_bytes.read().decode('utf-8')
loaded_model = tf.keras.models.model_from_json(model_architecture)

# Fetch the raw content of the model weights file
weights_url = raw_model_url.replace('.keras', '_weights.h5')
model_weights = requests.get(weights_url)
weights_bytes = BytesIO(model_weights.content)
